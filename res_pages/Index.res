...
@react.component
let make = (~text=textEn, ~data=data) =>
  <>
  <div className="lg:relative">
    <div className="mx-auto max-w-7xl w-full pt-16 pb-20 text-center lg:py-48 lg:text-left">
      <div className="px-4 lg:w-1/2 sm:px-8 xl:pr-16">
        <h1 className="text-4xl tracking-tight font-extrabold text-gray-900 sm:text-5xl md:text-6xl lg:text-5xl xl:text-6xl">{s(textEn["heroHeader"])}</h1>
        <p className="mt-3 max-w-md mx-auto text-lg text-gray-500 sm:text-xl md:mt-5 md:max-w-3xl">{s(textEn["heroBody"])}</p>
        <div className="mt-10 sm:flex sm:justify-center lg:justify-start">
          <div className="rounded-md shadow">
            <a href="#" className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-orangedark hover:bg-orangedarker md:py-4 md:text-lg md:px-10"> {s(textEn["installOcaml"])} </a>
          </div>
          <div className="mt-3 rounded-md shadow sm:mt-0 sm:ml-3">
            <a href="#" className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-orangedark bg-white hover:bg-gray-50 md:py-4 md:text-lg md:px-10"> {s(textEn["learnMore"])} </a>
          </div>
        </div>
      </div>
    </div>
  ...
  <div className="pt-12 sm:pt-16">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto text-center">
        <h2 className="text-3xl font-extrabold text-gray-900 sm:text-4xl">{s(text["statsTitle"])}</h2>
      </div>
    </div>
        ...
        <div className="max-w-4xl mx-auto">
          <dl className="rounded-lg bg-white shadow-lg sm:grid sm:grid-cols-3">
            <div className="flex flex-col border-b border-gray-100 py-16 text-center sm:border-0 sm:border-r">
              <dt className="order-2 mt-2 text-lg leading-6 font-bold text-black text-opacity-70">{s(text["activeMembers"])}</dt>
              <dd className="order-1 text-5xl font-extrabold text-orangedark">{s(data["activeMembers"])}</dd>
            </div>
            <div className="flex flex-col border-t border-b border-gray-100 py-16 text-center sm:border-0 sm:border-l sm:border-r">
              <dt className="order-2 mt-2 text-lg leading-6 font-bold text-black text-opacity-70">{s(text["industrySatisfaction"])}</dt>
              <dd className="order-1 text-5xl font-extrabold text-orangedark">{s(data["industrySatisfactionPercent"])}</dd>
            </div>
            <div className="flex flex-col border-t border-gray-100 py-16 text-center sm:border-0 sm:border-l">
              <dt className="order-2 mt-2 text-lg leading-6 font-bold text-black text-opacity-70">{s(text["averagePRsPerWeek"])}</dt>
              <dd className="order-1 text-5xl font-extrabold text-orangedark">{s(data["averagePRsPerWeek"])}</dd>
            </div>
          </dl>
        </div>
      </div>
